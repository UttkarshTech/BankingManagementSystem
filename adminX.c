...
void AdminMenu_server(int client_fd, struct User *curUserPtr){
    ...
    while (choice) {
        ...
        switch (choice){
            ...
            case 9 : {
                int flag = 0;
                struct User newEmployee = {.role = RE, .isActive = 1, .isLoggedIn = 0, .balance = 0.0 };
                ...
                if (flag == 1){
                    ...
                    strcpy(newEmployee.password, "default");
                    flag = addNewCustomer(&newEmployee);
                    send(client_fd, &flag, sizeof(flag), 0);
                }
            }
            break;
    ...
    while (choice){
        ...
        switch (choice){
            ...
            case 9 : { 
                int flag = 0;
                char username[MAXSTR], firstName[MAXSTR], lastName[MAXSTR];
                printf("Enter username for new employee : ");
                scanf("%s", username);
